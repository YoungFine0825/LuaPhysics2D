...
//----------------------------------------------------------------------------------
// Module Functions Declaration 公开方法
//----------------------------------------------------------------------------------
...
#define PHYSAC_FLT_MAX      3.402823466e+38f//浮点数最大长度
#define PHYSAC_EPSILON      0.000001f
#define PHYSAC_K            1.0f/3.0f //3分之一
#define PHYSAC_VECTOR_ZERO  (Vector2){ 0.0f, 0.0f }//2维零向量
...
typedef struct PhysicsShape {
    PhysicsShapeType type;                      // 类型枚举。Physics shape type (circle or polygon)
    PhysicsBody body;                           // Shape physics body reference
    float radius;                               // 圆形半径，主要用圆形。Circle shape radius (used for circle shapes)
    Matrix2x2 transform;                        // 2x2的顶点变换矩阵。Vertices transform matrix 2x2
    PolygonData vertexData;                     // 形状包含的顶点、法线数据。Polygon shape vertices position and normals data (just used for polygon shapes)
} PhysicsShape;

//物理实体体数据
typedef struct PhysicsBodyData {
    unsigned int id;                            // 引用唯一标识符 Reference unique identifier
    bool enabled;                               // 是否启用动力学。Enabled dynamics state (collisions are calculated anyway)
    Vector2 position;                           // 当前世界坐标。Physics body shape pivot
    Vector2 velocity;                           // 当前线性速度应用于位置 Current linear velocity applied to position
    Vector2 force;                              // 当前线性力（每帧清0） Current linear force (reset to 0 every step)
    float angularVelocity;                      // 当前角速度应用于定向 Current angular velocity applied to orient
    float torque;                               // 当前角距（每帧清0） Current angular force (reset to 0 every step)
    float orient;                               // 旋转弧度 Rotation in radians
    float inertia;                              // 转动惯量 Moment of inertia
    float inverseInertia;                       // 转动惯量映射到0~1范围后的值 Inverse value of inertia
    float mass;                                 // 质量 Physics body mass
    float inverseMass;                          // 质量映射到0~1范围后的值 Inverse value of mass
    float staticFriction;                       // 物体静止时的摩擦力（值范围0~1） Friction when the body has not movement (0 to 1)
    float dynamicFriction;                      // 物体运动时的摩擦力（值范围0~1））Friction when the body has movement (0 to 1)
    float restitution;                          // 恢复系数（两个物体碰撞除法线方向上的分离速度，值范围0~1）Restitution coefficient of the body (0 to 1)
    bool useGravity;                            // 是否应用重力Apply gravity force to dynamics
    bool isGrounded;                            // 是否站立与其他物体的表面 Physics grounded on other body state
    bool freezeOrient;                          // 是否约束旋转 Physics rotation constraint
    PhysicsShape shape;                         // 多边形信息（包含类型枚举，半径、顶点集合、法线集合）。Physics body shape information (type, radius, vertices, normals)
} PhysicsBodyData;

//物理流形数据（可以理解为保存两个实体碰撞时的相关信息）
typedef struct PhysicsManifoldData {
    unsigned int id;                            // 唯一标识符。Reference unique identifier
    PhysicsBody bodyA;                          // 参与碰撞的实体A。Manifold first physics body reference
    PhysicsBody bodyB;                          // 参与碰撞的实体B。Manifold second physics body reference
    float penetration;                          // 碰撞深度。Depth of penetration from collision
    Vector2 normal;                             // 从a到b的法线方向向量。Normal direction vector from 'a' to 'b'
    Vector2 contacts[2];                        // 碰撞接触点。Points of contact during collision
    unsigned int contactsCount;                 // 碰撞接触点数量。Current collision number of contacts
    float restitution;                          // 碰撞时的混合恢复系数。Mixed restitution during collision
    float dynamicFriction;                      // 碰撞时的混合静态摩擦力。Mixed dynamic friction during collision
    float staticFriction;                       // 碰撞时的混合动态摩擦力。Mixed static friction during collision
...
static bool physicsThreadEnabled = false;                   // 是否应用线程。Physics thread enabled state
static double baseTime = 0.0;                               // 单调时钟的偏移时间。Offset time for MONOTONIC clock
static double startTime = 0.0;                              // 开始时间（毫秒）。Start time in milliseconds
static double deltaTime = 1.0/60.0/10.0 * 1000;             // 每个物理帧时间增量（毫秒）。Delta time used for physics steps, in milliseconds
static double currentTime = 0.0;                            // 当前时间（毫秒）。Current time in milliseconds
static unsigned long long int frequency = 0;                // 高分辨率的时钟频率。Hi-res clock frequency

static double accumulator = 0.0;                            // 时间增量累加器。Physics time step delta time accumulator
static unsigned int stepsCount = 0;                         // 总共物理帧数量。Total physics steps processed
static Vector2 gravityForce = { 0.0f, 9.81f };              // 重力。Physics world gravity force
...
//----------------------------------------------------------------------------------
// Module Internal Functions Declaration 内部方法声明
//----------------------------------------------------------------------------------
static int FindAvailableBodyIndex();                                                                        // 为初始化物理实体寻找一个可用的Index用作唯一标识符。Finds a valid index for a new physics body initialization
...
static void *PhysicsLoop(void *arg);                                                                        // 用于线程的循环函数。Physics loop thread function
static void PhysicsStep(void);                                                                              // 每个物理帧计算动力学、碰撞、位置修正。Physics steps calculations (dynamics, collisions and position corrections)
static int FindAvailableManifoldIndex();                                                                    // 为初始化流形数据块寻找一个可用的Index作为唯一标识符。Finds a valid index for a new manifold initialization
static PhysicsManifold CreatePhysicsManifold(PhysicsBody a, PhysicsBody b);                                 // 创一个流形数据块用于处理碰撞。Creates a new physics manifold to solve collision
static void DestroyPhysicsManifold(PhysicsManifold manifold);                                               // 销毁流形数据。Unitializes and destroys a physics manifold
static void SolvePhysicsManifold(PhysicsManifold manifold);                                                 // 处理两个物体构成的流形数据。Solves a created physics manifold between two physics bodies
static void SolveCircleToCircle(PhysicsManifold manifold);                                                  // 处理两个圆形物体构成的流形数据。Solves collision between two circle shape physics bodies
static void SolveCircleToPolygon(PhysicsManifold manifold);                                                 // 处理一个圆形物体与一个多边形物体的流形数据。Solves collision between a circle to a polygon shape physics bodies
static void SolvePolygonToCircle(PhysicsManifold manifold);                                                 // 处理一个多边形物体与一个圆形物体的流形数据。Solves collision between a polygon to a circle shape physics bodies
static void SolvePolygonToPolygon(PhysicsManifold manifold);                                                // 处理两个多边形构成的流形数据。Solves collision between two polygons shape physics bodies
static void IntegratePhysicsForces(PhysicsBody body);                                                       // 根据物体受力情况计算物体的速度。Integrates physics forces into velocity
static void InitializePhysicsManifolds(PhysicsManifold manifold);                                           // 初始化指定流形数据块以处理碰撞。Initializes physics manifolds to solve collisions
static void IntegratePhysicsImpulses(PhysicsManifold manifold);                                             // 整合物体受到的推力以处理碰撞。Integrates physics collisions impulses to solve collisions
static void IntegratePhysicsVelocity(PhysicsBody body);                                                     // 根据物体的速度计算当前位置。Integrates physics velocity into position and forces
static void CorrectPhysicsPositions(PhysicsManifold manifold);                                              // 基于流传碰撞数据修正物体的位置。Corrects physics bodies positions based on manifolds collision information
static float FindAxisLeastPenetration(int *faceIndex, PhysicsShape shapeA, PhysicsShape shapeB);            // 找到多边形形状轴最小穿透。（来自百度翻译）Finds polygon shapes axis least penetration
static void FindIncidentFace(Vector2 *v0, Vector2 *v1, PhysicsShape ref, PhysicsShape inc, int index);      // 找出两个多边形的碰撞面。Finds two polygon shapes incident face
static int Clip(Vector2 normal, float clip, Vector2 *faceA, Vector2 *faceB);                                // 基于一个法线和两个面计算剪裁。Calculates clipping based on a normal and two faces
static bool BiasGreaterThan(float valueA, float valueB);                                                    // 检查值是否处于偏置区间。Check if values are between bias range
static Vector2 TriangleBarycenter(Vector2 v1, Vector2 v2, Vector2 v3);                                      // 返回由三个点给定的三角形重心。Returns the barycenter of a triangle given by 3 points
...
static float MathLenSqr(Vector2 vector);                                                                    // 返回一个向量长度平方根。Returns the len square root of a vector
static float MathDot(Vector2 v1, Vector2 v2);                                                               // Returns the dot product of two vectors
static inline float DistSqr(Vector2 v1, Vector2 v2);                                                        // 返回两个向量之间距离的平方根。Returns the square root of distance between two vectors
...
static Matrix2x2 Mat2Radians(float radians);//根据给定的弧度值创建一个2x2矩阵                                                                     // Creates a matrix 2x2 from a given radians value
static void Mat2Set(Matrix2x2 *matrix, float radians);                                                           // Set values from radians to a created matrix 2x2
static inline Matrix2x2 Mat2Transpose(Matrix2x2 matrix);//返回一个给定2x2矩阵的转置矩阵                                                              // Returns the transpose of a given matrix 2x2
static inline Vector2 Mat2MultiplyVector2(Matrix2x2 matrix, Vector2 vector);//计算一个2x2矩阵与一个二维向量的乘积                                     // Multiplies a vector by a matrix 2x2

//----------------------------------------------------------------------------------
// Module Functions Definition 公开方法定义
//----------------------------------------------------------------------------------
...
PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(Vector2 pos, float width, float height, float density)
{
    ...
    if (newId != -1)
    {
		...
		//计算矩形的质心与转动惯量。
        Vector2 center = { 0.0f, 0.0f };//求多边形质心
        float area = 0.0f;//求整个多边形的面积。
        float inertia = 0.0f;//求多边形转动惯量

        for (int i = 0; i < newBody->shape.vertexData.vertexCount; i++)
        {
            // Triangle vertices, third vertex implied as (0, 0)
			//多边形的顶点两两组成一个三角形，三角形第三个默认为原点。
            ...
            float D = MathCrossVector2(p1, p2);//两个二维向量的叉乘的值是这两向量为邻边的组成的平行四边形的面积。
            float triangleArea = D/2;//平行四边形的面积的一半以为这两个二维向量组成的三角形的面积 。

            area += triangleArea;//累加每一个三角形的面积。

            // Use area to weight the centroid average, not just vertex position.使用面积来确定质心的平均权重，而不仅仅是顶点的位置。
			// 计算三角形的质心
			//（p1+p2)的结果是这两个向量作为邻边组成的平行四边形的对角线,所以三角形的质心 = 三角形的面积 * 1/3 * 对角线
			//由于Vector2结构没有重载运算符，所以以下公式等同为：
			//center += triangleArea * ( 1 / 3 ) * (p1 + p2)			
        ...
        //由于多边形的各个顶点坐标是在世界坐标系下创建的，所以这里可以将各个顶点的坐标转换到该多边形的局部坐标系下。（但并非必需）
        for (int i = 0; i < newBody->shape.vertexData.vertexCount; i++)
        {
        ...
        //为实体各项物理属性赋值
        newBody->mass = density*area;//质量=密度*面积
        //将质量的值映射到0~1区间
        newBody->inverseMass = ((newBody->mass != 0.0f) ? 1.0f/newBody->mass : 0.0f);
        //多边形的转动惯量=密度*多边形中各个三角形转动惯量之和
        newBody->inertia = density*inertia;
        //将转动惯量的值映射到0~1区间
        newBody->inverseInertia = ((newBody->inertia != 0.0f) ? 1.0f/newBody->inertia : 0.0f);
        newBody->staticFriction = 0.4f;//默认静态摩擦力为0.4
        newBody->dynamicFriction = 0.2f;//默认动态摩擦力为0.2
        newBody->restitution = 0.0f;//默认恢复系数为0
        newBody->useGravity = true;//默认使用重力
...
PHYSACDEF void DestroyPhysicsBody(PhysicsBody body)
{
    if (body != NULL)
    {
        int id = body->id;//取给定Body的id，
        int index = -1;

        //由于所有Body都保存在一个数组中，所以需要寻找给定的Body在数组中的位置（即下标）。
        ...
        #endif
            return;     //Body数组中不存在给定的Body // Prevent access to index -1
        }
...
{
    int index = -1;
    //从0开始寻找一个所有已存在的Body都未使用的数字作为新创建的Body的Id
...
static PolygonData CreateRectanglePolygon(Vector2 pos, Vector2 size)
{
    ...
    for (int i = 0; i < data.vertexCount; i++)
    {
        //二维环境下，后一个顶点减去当前顶点（之所要用后一个见前一个，使用顶点时按照顺时针顺序创建的），得到一个表示两个顶点组成的面的向量
        int nextIndex = (((i + 1) < data.vertexCount) ? (i + 1) : 0);
        Vector2 face = Vector2Subtract(data.positions[nextIndex], data.positions[i]);
        //将面向量x、y轴的值互换，再对y轴的值取反，结果将垂直与该面向量，即为法线。
        data.normals[i] = (Vector2){ face.y, -face.x };
        //将法线归一化
        MathNormalize(&data.normals[i]);
    }
...
static void SolveCircleToCircle(PhysicsManifold manifold)
{
    ...
    float distSqr = MathLenSqr(normal);//求向量的模平方根
...
{
    PhysicsBody bodyA = manifold->bodyA;//BodyA为圆形
    PhysicsBody bodyB = manifold->bodyB;//BodyB为多边形
    ...
    // 因为任何Body都有可能发生旋转，所以需要将BodyA的坐标从世界空间转换到BodyB的局部空间下
    ...
    // 查找BodyB中与BodyA相交深度最大的那条边
    // It is the same concept as using support points in SolvePolygonToPolygon
    float separation = -PHYSAC_FLT_MAX;
    int faceNormal = 0;
    PolygonData vertexData = bodyB->shape.vertexData;
    //遍历BodyB中的顶点和面法线，找到发生碰撞的且相交深度最大的那一个面，并记录这个面的法线的索引
    for (int i = 0; i < vertexData.vertexCount; i++)
    {
        //相交深度 = 当前面的法线与当前面中的顶点到BodyA位置的方向向量的点乘
        ...
        if (currentSeparation > separation)
        {
            ...
            faceNormal = i;//保存法线在数组中的索引
        }
    }

    // Grab face's vertices
    // 获取碰撞面的两个顶点
    Vector2 v1 = vertexData.positions[faceNormal];//碰撞面的第一个端点
    int nextIndex = (((faceNormal + 1) < vertexData.vertexCount) ? (faceNormal + 1) : 0);
    Vector2 v2 = vertexData.positions[nextIndex];//碰撞面的第二个端点。

    // Check to see if center is within polygon
    // 检查BodyA的位置是否位于BodyB内
    if (separation < PHYSAC_EPSILON)
    {
        manifold->contactsCount = 1;
        //将BodyB被碰撞的那一面的法线向量取反，然后作为BodyA与BodyB发生碰撞时的方向
        Vector2 normal = Mat2MultiplyVector2(bodyB->shape.transform, vertexData.normals[faceNormal]);
        manifold->normal = (Vector2){ -normal.x, -normal.y };//对BodyB的面法线向量取反，就变成了从BodyA指向BodyB的向量.
        //再用BodyA的半径对法线进行缩放在加上BodyA的位置就得到碰撞点坐标
        manifold->contacts[0] = (Vector2){ manifold->normal.x*bodyA->shape.radius + bodyA->position.x, manifold->normal.y*bodyA->shape.radius + bodyA->position.y };
        //BodyA的半径即为BodyA中点到碰撞点的距离
        manifold->penetration = bodyA->shape.radius;
        return;
    }

    // Determine which voronoi region of the edge center of circle lies within
    //voronoi region ： 泰森多边形
    // 下面是检测BodyA的相对于碰撞面的位置。
    float dot1 = MathDot(Vector2Subtract(center, v1), Vector2Subtract(v2, v1));//dot1的几何意义为BodyA以碰撞面第一个端点v1为原点与碰撞面的夹角
    float dot2 = MathDot(Vector2Subtract(center, v2), Vector2Subtract(v1, v2));//dot2的几何意义为BodyA以碰撞面第二个端点v2为原点与碰撞面的夹角

    //最终碰撞深度等于BodyA中点到碰撞点的距离，即BodyA的半径 - 碰撞深度
    manifold->penetration = bodyA->shape.radius - separation;

    if (dot1 <= 0.0f) // Closest to v1 //BodyA的中心没有垂直与碰撞面且更靠近v1端点
    {
        //如果BodyA中心点到v1端点的距离大于BodyA的半径，则没有发生碰撞
    ...
    else if (dot2 <= 0.0f) // Closest to v2  //BodyA的中心没有垂直与碰撞面且更靠近v2端点
    {
        //如果BodyA中心点到v2端点的距离大于BodyA的半径，则没有发生碰撞
    ...
    else // Closest to face  //BodyA的中心垂直与碰撞面
    {
        Vector2 normal = vertexData.normals[faceNormal];//碰撞面法线
...
static void SolvePolygonToPolygon(PhysicsManifold manifold)
{
    ...
    // 取BodyA中与BodyB发生碰撞的顶点的索引，以及碰撞深度
    int faceA = 0;
    ...
    // 取BodyB中与BodyA发生碰撞的顶点的索引，以及碰撞深度
    int faceB = 0;
    float penetrationB = FindAxisLeastPenetration(&faceB, bodyB, bodyA);
    if (penetrationB >= 0.0f) return;

    int referenceIndex = 0;
    bool flip = false;  // Always point from A shape to B shape 为false表示是BodyA指向BodyB。

    PhysicsShape refPoly; // Reference 碰撞物
    PhysicsShape incPoly; // Incident 被碰撞物

    // Determine which shape contains reference face 确定哪个多边形包含参考面。
    // 比较碰撞深度大小，深度较大的那个多边形是碰撞物，另一个则是被碰撞物。
    if (BiasGreaterThan(penetrationA, penetrationB))
    {
        refPoly = bodyA;//BodyA是碰撞物
        incPoly = bodyB;//BodyB是被碰撞物
        referenceIndex = faceA;//BodyA中发生碰撞的那条边的索引
    }
    else
    {
        refPoly = bodyB;//BodyB是碰撞物
        incPoly = bodyA;//BodyA是被碰撞物
        referenceIndex = faceB;//BodyB中发生碰撞的那条边的索引
        flip = true;
    }

    // World space incident face
    // 查找被碰撞物中参与碰撞的那条边的两个顶点。
    ...
    // Orthogonalize
    // 对碰撞边正交化，得到碰撞物的碰撞边的法线。
    Vector2 refFaceNormal = { sidePlaneNormal.y, -sidePlaneNormal.x };
    float refC = MathDot(refFaceNormal, v1);//边法线与边的第一个顶点的夹角
...
// 遍历多边形的所有顶点，取与给方向向量夹角最小的那个顶点。
//参数dir向量所在的坐标系必须是参数Shape的局部坐标系。
...
// Finds polygon shapes axis least penetration 找到多边形形状的轴线最小穿透
// 遍历BodyA的所有顶点，判断那个顶点与BodyB发生碰撞。
...
static void FindIncidentFace(Vector2 *v0, Vector2 *v1, PhysicsShape ref, PhysicsShape inc, int index)
{
    ...
    Vector2 referenceNormal = refData.normals[index];//取碰撞物的法线

    // Calculate normal in incident's frame of reference
    //将反射方Body的法线转换到入射方Body的局部空间下。
    ...
    //遍历入射方Body的所有法线，取与反射方法线夹角最大的那一条法线。
